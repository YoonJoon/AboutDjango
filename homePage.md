...
이제 [LocalLibrary](tutorialLocalLibraryWebsite.md) 웹사이트의 첫번째 페이지인 홈 페이지 전체를 보여주는 코드를 추가할 준비가 되었습니다. 홈 페이지에서 각 모델의 레코드 수를 표시하고 다른 페이지로 연결되는 사이드바 탐색 링크를 제공합니다. 이 단계를 따라가면 기본 URL 맵과 뷰 작성, 데이터베이스에서 레코드 탐색 및 템플리트 사용의 실질적인 경험을 쌓을 수 있을 것입니다.

> 선수지식: [Django 소개](introduction.md)를 읽고 [Django 튜토리얼 4부: Django 관리 사이트](adminSite.md)까지 모든 단원들을 숙이해하고 있어야 합니다.
>
> 목표: (URL에 인코딩된 데이터가 없는) 간단한 URL 맵과 뷰를 만들고, 모델에서 데이터를 가져오고 그리고 템플리트를 만드는 법을 배울 수 있습니다.

---

### 개요

모델을 정의하고 앞으로 필요한 초기 라이브러리 레코드들을 삽입한 다음, 사용자에게 저장된 정보를 보여주는 코드를 작성해야 합니다. 가장 먼저해야 할 일은 페이지에서 보여줄 정보를 결정하고 해당 리소스를 반환할 URL을 정의하는 것입니다. 이후 페이지를 디스플레이하기 위한 URL 매퍼, 뷰와 템플리트를 생성합니다.

아래 그림에서는 주요한 데이터 흐름 및 HTTP 요청과 응답 처리에 필요한 구성 요소들을 설명합니다. 이미 모델을 구현했으므로 작성할 주요 구성 요소는 다음과 같습니다.

-	지원되는 URL(과 URL에 인코딩된 정보)을 적절한 뷰 함수에 전달하는 URL 맵퍼.
-	뷰 함수를 사용하여 모델에서 요청된 데이터를 추출하여, 데이터를 표시할 HTML 페이지를 작성하고, 이를 브라우저에서 사용자에게 보여줄 수 있도록 페이지를 반환합니다.
-	데이터를 뷰에서 렌더링할 때 사용할 템플리트입니다.

![](Pics/basicDjango.png)

다음 섹션에서 다루겠지만, 다섯 페이지를 출력합니다. 이는 한 단원에서 다루기에 너무 많은 내용입니다. 따라서 이 단원에서는 홈 페이지 구현 방법을 중점적으로 다루며 다음 단원에서 다른 페이지들을 다룰 것입니다. 그럼으로써 URL 매퍼, 뷰와 모델의 실제 작동을 완벽하게 이해할 수 있을 것입니다.

---

### URL 리소스 정의

[LocalLibrary](tutorialLocalLibraryWebsite.md)의 이 버전은 기본적으로 최종 사용자에게 읽기 전용이므로 사이트 의 시작 페이지(홈 페이지)와 도서와 저자의 목록 및 상세보기를 보여주는 페이지를 제공하면 됩니다.

페이지들을 위한 URL은 다음과 같습니다.

-	<code>catalog/</code> — 홈 (인덱스) 페이지
-	<code>catalog/books/</code> — 모든 도서 목록
-	<code>catalog/authors/</code> — 모든 저자 목록
-	<code>catalog/book/<i>\<id\></i></code> - 프라이머리 키 필드 값이 <i>\<id\></i>인 (기본값) 특정 도서에 대한 세부 상세 뷰입니다. 예를 들어 목록의 세 번째 책의 URL은 <code>/catalog/book/3</code>입니다.
...
반대로 마지막 두 URL은 도서나 저자에 대한 자세한 정보를 표시합니다. 이 URL에는 디스플레할 항목을 (위에서 <id>로 표시된) ID로 인코딩합니다 . URL 매퍼는 인코딩된 정보를 추출하여 이를 뷰로 전달하며, 뷰는 데이터베이스를 검색하여 동적으로 정보를 결정합니다. URL에 정보를 인코딩함으로써 모든 도서(또는 저자)를 처리하기 위한 URL 맵핑, 뷰와 템플리트의 단일 조합을 사용할 수 있습니다.

> <b>Note</b>: Django를 사용하여 위와 같이 URL 본문에 정보를 인코딩하거나 <code>/book/?id=6</code>와 같이 URL에 <code>GET</code> 매개 변수를 포함하여 URL을 작성할 수 있습니다. 어떤 방법을 택하든 [W3C 권장](https://www.w3.org/Provider/Style/URI)에 따라 URL을 깔끔하고 논리적이며 읽기 쉬운 상태로 유지해야 합니다.<br> Django 문서는 보다 나은 URL 디자인을 위해 URL 본문에 정보를 인코딩하는 방법을 권장합니다.

개요에서 언급했듯이 이 단원의 나머지 부분에서는 인덱스 페이지 작성 방법에 대해 설명합니다.

---

### 인덱스 페이지 작성

첫 번째 페이지는 인덱스 페이지(catalog/)입니다. 인덱스 페이지에는 데이터베이스에 저장된 다른 레코드 "수"와 함께 일부 정적 HTML을 포함하고 있습니다. 이 작업을 수행하기 위해 URL 매핑, 뷰와 템플리트를 생성합니다.

> <b>Note</b>: 이 섹션에 약간의 주의를 기울이십시오. 생성할 다른 페이지에도 대부분의 정보를 적용할 수 있기 때문입니다.

#### URL 맵핑

[웹사이트 골조 만들기](skeletonWebsite.md)에서 생성하였을 때 <code>catalog/</code>로 시작하는 URL이 수신될 때마다 <i>URLConf</i> 모듈의 <code>catalog.urls</code>에서 나머지 부분 문자열을 처리하도록 <b>locallibrary/urls.py</b> 파일을 업데이트했습니다.
...
<i>URLConf</i> 모듈을 위해 <b>/catalog/urls.py</b>라는 플레이스홀더 파일도 만들었습니다. 그 파일에 아래 문장을 추가합니다.
...
path() 함수는 name 매개 변수도 지정합니다.이 매개 변수는 URL 매핑의 고유 식별자입니다. 이 이름을 사용하여 매퍼를 역으로 지정할 수 있습니다. 즉, 매퍼가 처리하도록 설계된 리소스를 가리키는 URL을 동적으로 만들 수 있습니다. 예를 들어 name 매개 변수를 사용하여 템플리트에 다음 링크를 추가하여 다른 페이지에서 홈 페이지로 연결할 수 있습니다.

```python
<a href="{% url 'index' %}">Home</a>.
```

> <b>Note</b>: <code>\<a href="/catalog/">Home\<\/a></code>과 같이 링크를 하드 코딩할 수 있지만 홈 페이지의 패턴을 변경하면 (예: <code>/catalog/index</code>) 템플리트는 정확하지 않은 링크를 갖게 됩니다. 역방향 URL 매핑을 사용하면 훨씬 더 유연하고 영속성을 가질 수 있습니다.

#### <a name="View_(function-based)">(함수 기반) 뷰</a>

뷰는 HTTP 요청을 처리하고, 데이터베이스에 저장된 요청된 데이터를 접근하여 HTML 템플리트를 사용하여 HTML 페이지에 데이터를 렌더링한 다음 사용자에게 페이지를 보여주기 위하여 생성된 HTML을 HTTP 응답으로 반환하는 함수입니다. 인덱스 뷰는 이 모델을 따라 데이터베이스에 있는 <code>Book</code>, <code>BookInstance</code>, 이용 가능한 <code>BookInstance</code> 및 <code>Author</code>의 레코드 수에 대한 정보를 접근하여 이를 템플리트에 전달하여 표시합니다.

<b>catalog/views.py</b>를 열어 [render()](https://docs.djangoproject.com/en/2.1/topics/http/shortcuts/#django.shortcuts.render) 단축키 함수를 파일에서 import하고 템플리트와 데이터를 사용하여 HTML 파일 생성을 확인합니다.
...
다음 코드는 <b>base_generic.html</b> 파일의 기본 템플리트 샘플입니다. 샘플은 제목, 사이드바 및 <code>{% block title %}</code>과 </code>{% endblock %}</code> 템플리트 태그로 표시된 본문이 있는 공통 HTML을 포함하고 있습니다. 블록을 비워 두거나 템플리트에서 파생된 페이지를 렌더링할 때 사용할 기본 내용을 채울 수 있습니다.

> <b>Note</b>: 템플리트 <i>태그</i>는 템플리트에서 목록을 리스트를 사용하는 반복과 변수 값 기반의 조건부 연산 수행 등에 사용할 수 있는 함수입니다. 템플리트 태그 이외에도 템플리트 구문을 사용하면 뷰에서 템플리트로 전달되는 변수를 참조하고 (예 : 문자열을 소문자로 변환하는 것과 같이) <i>템플리트 필터</i>를 사용하여 변수를 포맷할 수 있습니다.
...
특정 뷰에 대한 템플리트를 정의할 때 먼저 <code>extends</code> 템플리트 태그를 사용하여 기본 템플리트를 지정합니다. 아래 코드 샘플을 참조하십시오. 다음 대체할 템플리트 섹션(있는 경우)을 기본 템플릿에서 처럼 <code>block/endblock</code> 섹션을 사용하여 선언합니다.
...
다음 코드를 <i>LocalLibrary</i> 웹사이트의 기본 템플리트으로 사용합니다. 보다시피, 여기에는 HTML 코드가 있으며, <code>title</code>, <code>sidebar</code>와 <code>content</code> 블록들을 정의합니다. 모든 도서와 저자의 목록에 대한 링크 및 기본 제목과 기본 사이드바를 가지고 있으며, 이들을 이후 쉽게 변경할 수 있도록 블록으로 구성합니다.
...
또한 기본 템플리트는 스타일을 추가로 제공하는 로컬 CSS 파일(<b>styles.css</b>)을 참조합니다. <b>/locallibrary/catalog/static/css/</b>에 <b>styles.css</b> 파일을 만들고 다음 코드를 복사합니다.
...
<b>/locallibrary/catalog/templates/</b>에 새로운 HTML 파일 <b><i>index.html</i></b>을 만들고 다음 코드를 복사합니다. 이 코드는 첫 줄로 기본 템플리트를 확장하고, 템플리트의 기본 <code>content</code> 블록을 바꿔놓습니다.
...
> <b>Note</b>: 템플리트 변수와 템플리트 태그(함수)를 쉽게 구별할 수 있습니다. 변수는 이중 중괄호(<code>{{num_books}}</code>)로 묶으며 태그는 퍼센트 기호(<code>{% extends "base_generic.html" %}</code>)가 있는 단일 중괄호로 묶습니다.
...
> <b>Note</b>: 위의 샘플은 파일의 위치를 지정하지만 Django는 기본적으로 이 정보를 제공하지 않습니다. [웹사이트 골조를 만들기](skeletonWebsite.md)에서 글로벌 URL 매퍼(<code>/locallibrary/locallibrary/urls.py</code>)를 수정하여 파일을 제공하도록 개발 웹 서버를 구성하지만 운영할 때 파일을 서비스할 수 있도록 활성화해야 합니다. 나중에 이를 설명할 것입니다.
...
데이터베이스의 여러 레코드와 아직 생성되지 않은 다른 페이지에 연결할 수 있는 링크를 보여주는 HTML 페이지인 사이트의 홈 페이지를 방금 만들었습니다. 이를 위하여 URL 매퍼, 뷰, 모델에서의 데이터베이스 검색, 뷰에서 템플리트로 정보 전달 및 템플리트 생성과 확장에 대한 기본적인 것들을 학습했습니다.
