...
이는 사이트에 모델을 등록하는 가장 간단한 방법입니다. admin 사이트는 매우 유연하게 맞춤 설정이 가능합니다. 모델을 등록하는 다른 방법에 대해 자세히 설명할 것입니다.
...
-	각 모델은 모델의 메소드 <code>\_\_str\_\_()</code>로 만들어진 문자열로 식별되며, 편집을 위해 상세 뷰와 양식에 링크된 개별 레코드 목록을 갖고 있습니다. 기본적으로 이 뷰 맨 위에 레코드에 대한 대량 삭제를 위한 작업 메뉴가 있습니다.

-	레코드의 편집과 추가를 위한 모델 상세 레코드 양식에는 모델의 모든 필드를 선언 순서에 따라 세로로 배치하여 보여줍니다.

더욱 쉽게 사용할 수 있도록 인터페이스를 더 사용자가 설 할 수 있습니다. 가능한 예는 다음과 같습니다.
...
-	상세 뷰:
	...
	-	인라인 수정이 가능하도록 관련 필드를 레코드에 추가할 수 있습니다 (예: author 레코드를 작성하는 동안 서적 레코드를 추가하고 편집할 수 있는 기능).
...
이제 <code>Book</code>과 <code>BookInstance</code>을 위한 <code>ModelAdmin</code> 클래스를 추가합니다. 그리고 원래 등록을 주석 처리합니다.
...
<code>AuthorAdmin</code> 클래스를 아래 코드로 바꿉니다. 목록에 필드 이름이 보이도록 <i>튜플</i>에 선언된 순서대로 디스플레이합니다 (원래 모델에서 정의한 것과 동일한 이름).
